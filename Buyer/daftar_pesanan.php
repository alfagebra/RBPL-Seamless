...
// Ambil data produksi yang diajukan oleh user
$query = "SELECT nama_produk, jumlah, tanggal_mulai, catatan, created_at 
          FROM produksi_requests 
...
<div class="container">
  ...
  <div class="main-content">
    <h2>Daftar Produksi Diajukan</h2>
    <table>
      <thead>
        <tr>
          <th>Nama Produk</th>
        ...
        <?php if (mysqli_num_rows($result) > 0): ?>
          <?php while ($row = mysqli_fetch_assoc($result)): ?>
            <tr>
              ...
              <td data-label="Jumlah"><?= htmlspecialchars($row['jumlah']) ?></td>
              <td data-label="Tanggal Mulai"><?= htmlspecialchars($row['tanggal_mulai']) ?></td>
              <td data-label="Catatan"><?= htmlspecialchars($row['catatan']) ?></td>
              <td data-label="Pengajuan"><?= htmlspecialchars(date('d M Y', strtotime($row['created_at']))) ?></td>
            </tr>
          <?php endwhile; ?>
        <?php else: ?>
          <tr><td colspan="5" style="text-align:center;">Belum ada pengajuan produksi.</td></tr>
        <?php endif; ?>
      </tbody>
    </table>
  </div>
